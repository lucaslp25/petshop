...
public class PetDaoJDBC implements PetDao {
    ...
    public void insertPet(Pet pet) {
        ...
        String sql = "INSERT INTO pet " +
                "(nome, especie, raca, porte, cliente_id) " +
                "VALUES " +
                "(?,?,?,?,?)";
    ...
    public void updatePet(Pet pet) {
        ...
        String sql = "UPTADE pet " +
                "SET nome = ?, especie = ?, raca = ?, porte = ?, cliente_id = ? " +
                "WHERE id = ?";
    ...
    public Pet findById(Integer id) {
        ...
        try{
            String sql =
                    "SELECT pet.*, cliente.*, endereco.* " +    //Pega todas informações do que esta relacionado ao pet, cliente e endereço!
                    "FROM pet " +       //indica que é do pet
                    "INNER JOIN cliente ON pet.cliente_id = cliente.id " +  //inner join necessario para ter as associações no banco de dados!
            ...
            if(rs.next()){
                ...
                Cliente cliente = instantiateCliente(rs, endereco); //aqui ja recebe chave estrangeira do endereço por isso associamos!0
    ...
    public List<Pet> findByClienteId(Integer id) {
        ...
        ResultSet rs = null;
        try{
            String sql = "SELECT pet.*, cliente.*, endereco.* " +
                    "FROM pet  " +
    ...
    private Pet instantiatePet(ResultSet rs, Cliente cliente)throws SQLException{
        ...
        pet.setEspecie(TiposDeEspecies.valueOf(rs.getString("especie")));   //enums fica desse jeito a conversão
